<script>
  ...
  import updateCustomBarInfo from './tuniao-ui/libs/function/updateCustomBarInfo.js'
	...
	export default {
      ...
      // 获取设备的状态栏信息和自定义顶栏信息 //从store里面读取配置信息.这些配置信息会在各个vue里面共享,所以我们用vuex来保存.
      // store.dispatch('updateCustomBarInfo')
      updateCustomBarInfo().then((res) => { 
		  console.log("435435j34lk5j43lk5j34lkj5l3k4j")
		  console.log("res",res)
		  // commit 第一个参数是 触发的函数名, 后面是给这个函数的参数.
        store.commit('$tStore', {  // state是本身, payload是这个对象.
      ...
      })  //这行就是赋值. state.vuex_status_bar_height=res.statusBarHeight
	  //读取:
