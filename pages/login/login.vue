...
let timer;
	...
	methods: {
		...
		startIntervalFn() {
			...
			clearInterval(timer);      //防抖,都会每次清除上次timer
			timer = setInterval(() => { //每一秒都更改状态.
		...
		async sendSmFn() {
			let that = this;			
			if (that.sendIng) {
				return false;          //正在发送验证码时候, 不让这个按钮按下有反应. 防抖
			}
